    ...
    def _create_tokenizer(self, config):
        ...
        if self.special_tokens_cfg == 'QARCL' or self.special_tokens_cfg == 'QARC' or self.special_tokens_cfg == 'QARCD':
            if config == 'bert-base-uncased':
                tok.add_special_tokens({'additional_special_tokens': ['[unused1]', '[unused2]', '[unused3]']})
                special_tokens['C'] = '[unused1]'
                special_tokens['A'] = '[unused2]'
                special_tokens['R'] = '[unused3]'
            else:
                tok.add_special_tokens({'additional_special_tokens': ['[UNUSED_1]', '[UNUSED_2]', '[UNUSED_3]']})
                special_tokens['C'] = '[UNUSED_1]'
                special_tokens['A'] = '[UNUSED_2]'
                special_tokens['R'] = '[UNUSED_3]'
        else:
            if config == 'bert-base-uncased':
                tok.add_special_tokens({'additional_special_tokens': ['[unused1]']})
                special_tokens['D'] = '[unused1]'
            else:
                tok.add_special_tokens({'additional_special_tokens': ['[UNUSED_1]']})
                special_tokens['D'] = '[UNUSED_1]'
        ...
        if self.special_tokens_cfg == 'QARCL':
            batch_text = [
                divide_hunk_lines(x, context_tok=self.special_tokens['C'], added_tok=self.special_tokens['A'],
                                  removed_tok=self.special_tokens['R']) for x in batch_text]
            ...
            batch_text = [
                divide_hunk(x, context_tok=self.special_tokens['C'], added_tok=self.special_tokens['A'],
                            removed_tok=self.special_tokens['R'], concate=True) for x in batch_text]
            batch_text = [x.replace(';', ' [SEP] ') for x in batch_text]
        elif self.special_tokens_cfg == 'QARCD':
            batch_text = [
                divide_hunk(x, context_tok=self.special_tokens['C'], added_tok=self.special_tokens['A'],
                            removed_tok=self.special_tokens['R'], concate=False) for x in batch_text]
            ...
            batch_text = [self.special_tokens['D'] + ' ' + process_hunk(x) for x in batch_text]
            batch_text = [x.replace(';', ' [SEP] ') for x in batch_text]
...
def divide_hunk(hunk, context_tok='[UNUSED_1]', added_tok='[UNUSED_2]', removed_tok='[UNUSED_3]', concate=True):
    context = []
    added = []
    removed = []
...
def divide_hunk_lines(hunk, context_tok='[UNUSED_1]', added_tok='[UNUSED_2]', removed_tok='[UNUSED_3]'):
