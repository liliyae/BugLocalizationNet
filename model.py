...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger('fbl-bert')
random.seed(1234)
...
class ABERT(nn.Module):
    ...
    def _load_pretrained_bert(self):
        if 'codebert' in self.config:
            ...
            #self.config = "/home/sdu/dyl/bug/srcdistillation/SimpleBERTOverflow/"
            print(self.config)
            bert = BertModel.from_pretrained(self.config)
        return bert

    def _load_pretrained_bert_code(self):
        #tokenizer = AutoTokenizer.from_pretrained('/home/sdu/dyl/bug/graphcodebert')
        #bert = AutoModel.from_pretrained('/home/sdu/dyl/bug/graphcodebert')
        tokenizer = AutoTokenizer.from_pretrained('/home/sdu/dyl/pretrain/ABERT/mybert')
        bert = AutoModel.from_pretrained('/home/sdu/dyl/pretrain/ABERT/mybert')
        bert.resize_token_embeddings(len(tokenizer) + 4)
        return bert
    ...
    def forward(self, Q, D, bsize=16, tag="test"):
        if tag == "test":
            ...
            """ anchor = torch.mean(q_doc[q_doc.shape[0]/2,:], dim=1).squeeze()
            pos = torch.mean(k_query[k_query.shape[0]/2,:], dim=1).squeeze()
            l_pos = torch.einsum('nc,nc->n', [anchor, pos]).unsqueeze(-1)
            ...
            return self.score(p_query, p_doc), self.score(q_query, q_doc), logits1, labels1
