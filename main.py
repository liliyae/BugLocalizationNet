...
from fblbert import FblBERT, EagerBatcher
...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger('fbl-main')
...
def train(args, device):
    ...
    colbert = FblBERT(args.config, args.special_tokens, dev=device, query_maxlen=args.query_maxlen,
        ...
        for batch_idx, BatchSteps in zip(range(start_batch_idx, args.maxsteps), reader):
            ...
            for queries, passages in BatchSteps:
                with amp.context():
                    ...
                    x, y,  output1, target1 = colbert(queries, passages,args.bsize, tag="train")
                    ...
                    loss3 = criterion(output1, target1)
                    #loss = 0.4 * loss1 + 0.4 * loss2 + 0.2 * loss3
                    loss = 0.2 * loss1 + 0.8 * loss2 + 0.01 * loss3
